...
/**
 * This class handles disk and memory caching of bitmaps in conjunction with the {@link ImageLoader} class and its
 * subclasses. Use {@link ImageCache#getInstance(FragmentManager, ImageCacheParams)} to get an instance of this
 * class, although usually a cache should be added directly to an {@link ImageLoader} by calling
...
public class ImageCache {
	 ...
	 * called directly by other classes, instead use {@link ImageCache#getInstance(FragmentManager, ImageCacheParams)}
		...
		synchronized (mDiskCacheLock) {
			if (mDiskLruCache == null || mDiskLruCache.isClosed()) {
				...
				if (mCacheParams.diskCacheEnabled && diskCacheDir != null) {
					...
					Log.d(TAG, "get usable space");
					if (getUsableSpace(diskCacheDir) > mCacheParams.diskCacheSize) {
						Log.d(TAG, "usable space enough");
						try {
...
//							}
	...
	public static class ImageCacheParams {
		...
		public ImageCacheParams(String root, String diskCacheDirectoryName) {
			diskCacheDir = new File(root, diskCacheDirectoryName);
		}
		...
		/**
		 * @param size, kilobytes.
		 * @return
		 */
		public ImageCacheParams setMemCacheSize(int size) {
			memCacheSize = size;
			return this;
		}
		 ...
		 * @return
		 */
		public ImageCacheParams setDiskCacheSize(int size) {
			diskCacheSize = size * 1024;
			return this;
		}
	}
